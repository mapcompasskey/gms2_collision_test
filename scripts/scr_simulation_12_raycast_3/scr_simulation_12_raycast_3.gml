/// @function scr_simulation_12_raycast_3();
...
var _test_v = false;//(raycast_move_v == 0 ? false : true);
...
while ((_test_h || _test_v) && ! _collision_h && ! _collision_v)
{
    ...
    if (_test_h && ( ! _test_v || _ray_delta_h <= _ray_delta_v))
    {
        ...
        // if not the first horizontal test
        if ( ! _first_h)
        {
            ...
            // if the horizontal movement is negative, the entity is moving left, so shift the cell's x left by one
            _cell_x += (raycast_move_h < 0 ? -1 : 0);
        }
        
        /*
        // if the vertical point is on a vertical intersection
        // (and there is no slope or the other movement is negative)
        if (_step_h_y mod _cell_size == 0 && (_gradient == 0 || raycast_move_v < 0))
        {
            // if the slope is zero, then the ray is cast from the enities bottom but it needs to be shifted up by one since its resting on a cell
            // if the vertical movement is negative, the entity is moving up, so shift the cell's y up by one
            _cell_y += -1;
        }
        */
        
        // if the vertical point is on a vertical intersection and the vertical movement is negative
        if (_step_h_y mod _cell_size == 0 && raycast_move_v < 0)
        {
            // the entity is moving up, so shift the cell's y up by one
            _cell_y += -1;
        }
        ...
        //var _first_cell_y = _cell_y;
        //var _last_cell_y = floor((_step_h_y + bbox_height) / _cell_size);
        
        //_size_delta = 0;
        //_size_target = (bbox_height + _cell_size);
        
        var _cell_y1 = _cell_y;
        var _cell_y2 = floor((_step_h_y + ((bbox_height + 1) * (raycast_move_v > 0 ? -1 : 1))) / _cell_size);
        
        //if (raycast_move_v < 0 && _cell_y2 mod _cell_size == 0)
        //{
        //    _cell_y2 -= 1;
        //}
        
        //var _list = ds_list_create();
        //ds_list_add(_list, (_cell_x * _cell_size), (_cell_y1 * _cell_size), global.COLLISION_H_COLOR);
        ...
        //var _list = ds_list_create();
        //ds_list_add(_list, (_cell_x * _cell_size), (_cell_y2 * _cell_size), global.COLLISION_V_COLOR);
        //ds_list_add(global.GUI_AXIS_POINTS, _list);
        //ds_list_mark_as_list(global.GUI_AXIS_POINTS, ds_list_size(global.GUI_AXIS_POINTS) - 1);
        
        _size_delta = 0;
        _size_target = (abs(_cell_y1 - _cell_y2) * _cell_size);
    ...
    else if  (_test_v && ( ! _test_h || _ray_delta_v <= _ray_delta_h))
    {
        ...
        while (_size_delta < _size_target)
        {
            ...
            else if (_tile_at_point == _tile_slope_45_1 || _tile_at_point == _tile_slope_45_2)
            {
                ...
                if (scr_simulation_12_slope(_start_x, _start_y, _cell_x, _cell_y, _gradient, _ray_target_v, _tile_at_point))
                {
